...
class ModeAnalysis:
    ...
    def __init__(self, N=19, XR=3.082, 
        ...
        self.Nion = N
        self.initial_guess_3D = None # guess lattice for 3D crystal
        self.initial_guess = None # guess lattice for 2D crystal
        self.attempts_3D = np.linspace(.1, 1, 25)[::-1] # attempts to perturb 3D crystal
        self.attempts = np.linspace(.1, 1, 25)[::-1] # attempts to perturb 2D crystal
    ...
    def generate_crystal_3D(self): 
        ...
        #Generate a lattice in dimensionless units
        if self.initial_guess_3D is None:
            self.u0_3D = self.find_scaled_lattice_guess_3D()
        else:
            self.u0_3D = self.initial_guess_3D  
        ...
        if self.precision_solving is True:
            ...
            for attempt in self.attempts_3D:
